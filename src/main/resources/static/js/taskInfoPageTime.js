var flag = "1";//1新增 2修改
$(function(){
    var url = httpUrl + "/datax/task/list/?orderBy=create_time&remark=2&state=-1&type=-1&rerunCount=-1&id=-1";
...
function getPageStartInfo(resp){
    ...
    $.each(json, function (i, item) {
        var dealRate = item["dealRate"];//处理速率
        var dealingNum = item["dealingNum"];//已处理条数
        var deal = "";
        if(dealRate && dealingNum){
            dealRate = parseInt(dealRate);
            dealingNum = parseInt(dealingNum);
            var dealtime = (1000000 - dealingNum)/dealRate;
            dealtime = parseInt(dealtime);
            var times = hourTimes(dealtime);
            if(item["state"] == "1"){
                deal = "00:00:00";
            }else{
                if(dealRate == "0"){
                    ...
                    deal = times.hourVal + ":" + times.minVal + ":" + times.secVal;
                }
            }
        }
        json[i]["deal"] = deal;//剩余处理时间

        //实际完成时间
        var real = "";
        json[i]["real"] = real;
        if(item["endTime"] && item["startTime"]){
            if(item["state"] == "1"){//已完成计算 完成时间
                var timeSecond = (item["endTime"] - item["startTime"])/1000;
                var times = hourTimes(timeSecond);
                real = times.hourVal + ":" + times.minVal + ":" + times.secVal;
            }
            json[i]["real"] = real;//实际完成时间
        }
...
var columns = [
    ...
    {"field":"startTime","title":"任务开始时间","width":"120px","data":"startTime"},
    /*{"field":"endTime","title":"任务结束时间","width":"120px","data":"endTime"},*/
    {"field":"state","title":"状态","data":"state"},
    {"field":"errorInfo","title":"任务报错信息","width":"100px","data":"errorInfo"},
    {"field":"dealNum","title":"任务处理数据条数","data":"dealNum"},
    {"field":"deal","title":"预估完成时间","data":"deal"},
    {"field":"real","title":"实际完成时间","data":"real"},
